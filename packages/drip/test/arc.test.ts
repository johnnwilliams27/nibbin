...
/** In-memory DripStore with the same conflict rules as the M5 migration. */
function memoryStore(arcSeed: Omit<ArcRow, 'sends'>[], clock: () => Date) {
  const rows: StoreRow[] = [];
  ...
  const store: DripStore = {
    async activeArcs(): Promise<ArcRow[]> {
      return arcSeed
        .filter((a) => !completed.has(a.accountId))
        .map((a) => ({ ...a, sends: rows.filter((r) => r.accountId === a.accountId).map((r) => ({ ...r })) }));
    },
    async claimSend(accountId, beat, day): Promise<boolean> {
      // unique (account_id, beat)
      if (rows.some((r) => r.accountId === accountId && r.beat === beat)) return false;
      ...
      rows.push({ accountId, beat, status: 'claimed', localDay: day, claimedAt: clock() });
      return true;
    },
    ...
    async recordSkipped(accountId, beats, day) {
      for (const beat of beats) {
        if (rows.some((r) => r.accountId === accountId && r.beat === beat)) continue;
        rows.push({ accountId, beat, status: 'skipped', localDay: day, claimedAt: null });
      }
    },
  ...
  return { store, rows, notifications, completed };
}
...
describe('the 14-day arc, fast-clock (DoD)', () => {
  it('delivers the full no-Observer arc: right beats, right days, then completes', async () => {
    ...
    // The arc closed.
    expect(sim.mem.completed.has('acct-1')).toBe(true);
  });
