...
it('builds a Google authorization URL and persists pending state, requesting read + write scopes (Connector Lever 1)', async () => {
  ...
  // Connect now requests read AND the declared write scopes in one consent.
  ...
  // The exact requested scopes are recorded on the pending row.
  expect(saved[0].scopes).toContain('https://www.googleapis.com/auth/gmail.send');
  expect(saved[0].state).toBe(url.searchParams.get('state'));
});

it('Calendar connect requests calendar.readonly + calendar.events in one consent', async () => {
  const saved: StorePendingInput[] = [];
  const res = await beginConnect(
    { provider: 'google-calendar', accountId: 'a', userId: 'u', userEmail: 'john@gmail.com' },
    {
      config,
      allowlistFor: async () => makeTesterAllowlist(['john@gmail.com']),
      save: async (i) => { saved.push(i); },
