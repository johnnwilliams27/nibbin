...
function makeReq(authorization?: string): NextRequest {
  return new NextRequest('http://localhost/api/cron/source-extraction-reaper', {
    method: 'GET',
...
describe('GET /api/cron/source-extraction-reaper', () => {
  beforeEach(() => {
  ...
  it('calls reap_stale_extractions RPC and returns reaped count', async () => {
    rpcMock.mockResolvedValue({ data: 2, error: null });
    const { GET } = await import('./route');
    const res = await GET(makeReq(`Bearer ${SECRET}`));
    expect(res.status).toBe(200);
    const body = await res.json() as { ok: boolean; reaped: number };
    expect(body).toEqual({ ok: true, reaped: 2 });
    expect(rpcMock).toHaveBeenCalledWith('reap_stale_extractions', {});
  });

  it('returns ok:true with reaped:0 when no stale jobs', async () => {
