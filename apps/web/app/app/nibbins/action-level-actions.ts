...
export async function setNibbinActionLevel(
  ...
  if (level === 'send') {
    ...
    // → calendar.event-create (P2-1: a calendar Nibbin at send auto-executes
    // calendar.event-create, so the audit table must carry that grant too).
    ...
    const PROVIDER_WRITE_CAPABILITY: Record<string, 'email.send' | 'calendar.event-create'> = {
      gmail: 'email.send',
      'google-calendar': 'calendar.event-create',
    };

    if (activeConns && activeConns.length > 0) {
      const grantRows = activeConns
        .map((conn: { id: string; provider: string }) => {
          const capability = PROVIDER_WRITE_CAPABILITY[conn.provider];
