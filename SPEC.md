...
| C11 | "We never sell your data, and never train on it without explicit opt-in" | No third-party data sharing; model-improvement contribution is a separate, explicit, default-off opt-in. |
...
4. **Connect accounts.** Grovekeeper requests the user's top 2–4 connectors (ranked by answers), read-only scopes (C8). Each successful connection gets a micro-celebration (leaf confetti, journal scribble).
