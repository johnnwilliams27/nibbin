...
| C8 | "Nothing acts on your behalf while a Nibbin is still learning" | Connect requests the access your Nibbins may use (read + write scopes, explained plainly). Execution is gated by the earned-autonomy model: while a Nibbin is in School it drafts every side effect for your approval; it only acts autonomously once it has earned your trust (Graduate, or a Senior on a proven routine). Acquiring a scope never authorizes an unapproved action. |
