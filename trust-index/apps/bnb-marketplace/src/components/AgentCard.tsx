...
import { ArrowDownUp, ArrowUpRight, ChartNoAxesCombined, ChevronDown, Grid2X2, Shapes, ShieldCheck } from 'lucide-react';
...
import { COVERAGE_COPY, timestamp } from '@/lib/format';

const categoryIcons = { rebalancing: ArrowDownUp, grid_trading: Grid2X2, yield: ChartNoAxesCombined, health_factor: ShieldCheck, other: Shapes };
...
export function TrustIndexStatus({ agent, showLabel = true }: { agent: Agent; showLabel?: boolean }) {
  ...
  if (rating) {
    ...
    return <div className="flex flex-wrap items-baseline gap-x-3 gap-y-1 text-[14px]">
      <p aria-label={`Trust Index score: ${rating.score} out of 100`}>
        ...
        <span className="text-[22px] font-semibold tabular-nums">{rating.score}</span>
        <span className="text-[var(--fg-muted)]"> / 100</span>
      </p>
      <p className="text-[12px] text-[var(--coverage)]" title={coverage.meaning}>{coverage.label} coverage</p>
    </div>;
  }
