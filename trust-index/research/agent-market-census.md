...
|---|---|---|---|---|
| Official MCP registry | tool provider | yes | 12829 | 8240 |
...
**Official MCP registry** (tool provider). TRUNCATED at the --max row limit of 40000; the registry has more. This is a floor, not a total.
  version rows returned: 40000; distinct servers: 12829; servers exposing a remote endpoint: 8240; servers install-only: 4589
...
**Hugging Face Spaces** (mixed). Public, paginated, no key. Spaces expose a documented API at /gradio_api/info, so they are directly exercisable. 10122 carry the mcp-server tag and publish themselves as callable services; the total is at least 40000. A mix of genuine services and demos, but the callable subset alone is larger than every on-chain population combined.
  tagged mcp-server (callable as a service): 10122; gradio, which all expose /gradio_api/info: 30473; total spaces (capped, floor only): 40000
