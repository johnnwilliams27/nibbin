...
describe.skipIf(!dbAvailable)('Company Brain Foundation — F1 schema + RLS', () => {
  ...
  beforeAll(async () => {
    ...
    await h.as(asB, async (c) => { await c.query(`select public.create_account_with_owner('B')`); });
  });
...
describe.skipIf(!dbAvailable)('F2 — decide_memory_proposal apply-on-approve', () => {
  ...
  beforeAll(async () => {
    ...
    await h.as(asOther, async (c) => { await c.query(`select public.create_account_with_owner('F')`); });
  });
  afterAll(async () => { await h.close(); });
